...
for fichier in fichiers:
    ...
    # Lire l'Excel (les données commencent à la ligne 2, l'en-tête est à la ligne 1)
    df = pd.read_excel(fichier, header=1)
    print(f"   📊 {len(df)} lignes lues")
    
    documents = []
    for idx, row in df.iterrows():
        # Générer un identifiant unique sous forme de chaîne (24 hex)
        doc_id = str(ObjectId())
        
        # Récupérer la date et la convertir en chaîne (éviter 'nan')
        date_val = row.get('Date')
        if pd.notna(date_val):
        ...
        doc = {
            ...
            'Commentaire_Client': str(row.get('Commentaire Client', '')),
            'commentaire_moderateur': str(row.get('Commentaire Modérateur', '')),
            'date': date_str,                     # ← stocké en string
            'source': str(row.get('Réseau Social', '')),
            'moderateur': str(row.get('Modérateur', '')),
            'traite': False,                      # pour le traitement incrémental
            'metadata': {
                'fichier': os.path.basename(fichier),
                'ligne': idx + 2,
                'date_import': datetime.now()
