...
# # scripts/nettoyage/01_supprimer_urls_multinode_v3.py
# # VERSION VRAIMENT DISTRIBUÉE — mapPartitions
# # Spark 4.1.1 / Scala 2.13 / Java 21
...
# NB_WORKERS        = 2
# SPARK_MASTER      = "spark://spark-master:7077"
# RAPPORT_PATH      = "/home/mouna/projet_telecom/scripts/nettoyage/Nettoyage des textes/Rapports/rapport_nettoyage_complet.txt"
...
#     """
#     Chaque Worker :
#     1. Supprime URLs + @ + Ponctuations + HASHTAG
#     2. Se connecte directement à MongoDB
#     3. Écrit ses données nettoyées (ignore les lignes vides)
#     """
#     import sys
...
#         texte = re_ponctuation.sub(' ', texte)
#         texte = re_espaces.sub(' ', texte).strip()
#         return texte if texte else None
...
#         # ✅ Ignorer les lignes vides — ne pas les insérer
...
# print("="*70)
# print("🔍 NETTOYAGE COMPLET — URLs + @ + PONCTUATIONS + LIGNES VIDES")
# print("   Spark 4.1.1 | mapPartitions | Workers → MongoDB direct")
# print("="*70)
...
# spark = SparkSession.builder \
#     .appName("Nettoyage_Complet_MultiNode") \
#     .master(SPARK_MASTER) \
#     .config("spark.executor.memory", "2g") \
#     .config("spark.executor.cores", "2") \
#     .config("spark.sql.shuffle.partitions", "4") \
...
# df_analyse = df_spark \
#     .withColumn("a_url",   when(col("Commentaire_Client").rlike(PATTERN_DETECTION), 1).otherwise(0)) \
#     .withColumn("a_at",    when(col("Commentaire_Client").rlike(PATTERN_AT), 1).otherwise(0)) \
#     .withColumn("a_ponct", when(col("Commentaire_Client").rlike(PATTERN_DETECT_PONCT), 1).otherwise(0))
...
#     return df \
#         .withColumn(colonne, trim(regexp_replace(col(colonne), PATTERN_URLS, ""))) \
#         .withColumn(colonne, regexp_replace(col(colonne), PATTERN_AT, "")) \
#         .withColumn(colonne, regexp_replace(col(colonne), PATTERN_HASHTAG, "")) \
#         .withColumn(colonne, regexp_replace(col(colonne), PATTERN_PONCTUATION, " ")) \
#         .withColumn(colonne, trim(regexp_replace(col(colonne), PATTERN_ESPACES, " ")))

# df_nettoye = nettoyer_colonne(df_spark, "Commentaire_Client")
# df_nettoye = nettoyer_colonne(df_nettoye, "commentaire_moderateur")
# df_nettoye.cache()
...
#     .withColumn("urls_ap",  when(col("Commentaire_Client").rlike(PATTERN_DETECTION), 1).otherwise(0)) \
#     .withColumn("at_ap",    when(col("Commentaire_Client").rlike(PATTERN_AT), 1).otherwise(0)) \
#     .withColumn("ponct_ap", when(col("Commentaire_Client").rlike(PATTERN_DETECT_PONCT), 1).otherwise(0))

# stats_apres = df_apres.agg(
#     spark_sum("urls_ap").alias("avec_urls"),
#     spark_sum("at_ap").alias("avec_at"),
#     spark_sum("ponct_ap").alias("avec_ponct")
# ).collect()[0]

# avec_urls_ap  = int(stats_apres["avec_urls"]  or 0)
# avec_at_ap    = int(stats_apres["avec_at"]    or 0)
# avec_ponct_ap = int(stats_apres["avec_ponct"] or 0)

# temps_fin_nettoyage = time.time()

# taux_urls  = ((avec_urls_av  - avec_urls_ap)  / avec_urls_av  * 100) if avec_urls_av  > 0 else 100.0
# taux_at    = ((avec_at_av    - avec_at_ap)    / avec_at_av    * 100) if avec_at_av    > 0 else 100.0
# taux_ponct = ((avec_ponct_av - avec_ponct_ap) / avec_ponct_av * 100) if avec_ponct_av > 0 else 100.0
...
# print(f"   │ @ supprimés           : {avec_at_av - avec_at_ap:<15} │")
# print(f"   │ Taux @                : {taux_at:<14.2f}% │")
# print(f"   │ Ponct. supprimées     : {avec_ponct_av - avec_ponct_ap:<15} │")
# print(f"   │ Taux ponctuations     : {taux_ponct:<14.2f}% │")
# print(f"   └──────────────────────────────────────┘")

# # 6. ÉCRITURE DISTRIBUÉE
# print("\n💾 ÉCRITURE DISTRIBUÉE — Chaque Worker écrit dans MongoDB...")
# temps_debut_sauvegarde = time.time()
...
# total_vides    = sum(s.get("docs_vides", 0)   for s in stats_ecriture if s.get("statut") == "ok")
# erreurs        = [s for s in stats_ecriture if "_erreur" in s]
...
# urls_restantes  = coll_dest.count_documents({"Commentaire_Client": {"$regex": PATTERN_DETECTION, "$options": "i"}})
# at_restants     = coll_dest.count_documents({"Commentaire_Client": {"$regex": "@"}})
# ponct_restantes = coll_dest.count_documents({"Commentaire_Client": {"$regex": "[.,!?;:]"}})
...
# temps_fin_verif = time.time()
# succes = urls_restantes == 0 and at_restants == 0 and ponct_restantes == 0 and vides_restants == 0

# print(f"   • Documents en destination  : {total_en_dest}")
# print(f"   • URLs restantes            : {urls_restantes}")
# print(f"   • @ restants                : {at_restants}")
# print(f"   • Ponctuations restantes    : {ponct_restantes}")
...
# rapport = f"""
# {"="*70}
# RAPPORT — NETTOYAGE COMPLET (URLs + @ + Ponctuations + Lignes vides)
# {"="*70}
# Date   : {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}
# Mode   : Spark 4.1.1 | mapPartitions | Workers → MongoDB direct
...
#    • Nettoyage        : {temps_fin_nettoyage - temps_debut_nettoyage:.2f}s
#    • Écriture MongoDB : {temps_fin_sauvegarde - temps_debut_sauvegarde:.2f}s
#    • TOTAL            : {temps_total:.2f}s ({total/temps_total:.0f} doc/s)

# 📊 RÉSULTATS:
#    • Total source       : {total} documents
#    • URLs supprimées    : {avec_urls_av - avec_urls_ap} ({taux_urls:.1f}%)
#    • @ supprimés        : {avec_at_av - avec_at_ap} ({taux_at:.1f}%)
#    • Ponct. supprimées  : {avec_ponct_av - avec_ponct_ap} ({taux_ponct:.1f}%)
#    • Lignes vides       : {total_vides} ignorées (non insérées)
#    • Total inséré       : {total_inseres} documents

# 🧹 PONCTUATIONS SUPPRIMÉES:
#    • Latines  : . , ! ? ; : " ' « » ( ) [ ] {{ }}
#    • Arabes   : ؟ ، ؛
#    • Symboles : … – — ~ ` ^ * + = < > | \\ / _
...
# print(f"🗑️  URLs supprimées         : {avec_urls_av - avec_urls_ap} ({taux_urls:.1f}%)")
# print(f"🗑️  @ supprimés             : {avec_at_av - avec_at_ap} ({taux_at:.1f}%)")
# print(f"🗑️  Ponctuations supprimées : {avec_ponct_av - avec_ponct_ap} ({taux_ponct:.1f}%)")
...
PATTERN_URLS         = r'https?://\S*|www\.\S+'
PATTERN_AT           = r'@'
PATTERN_HASHTAG      = r'#'  # ✅ AJOUTÉ : pattern pour les hashtags
PATTERN_ESPACES      = r'\s+'
PATTERN_DETECTION    = r'https?://|www\.'
PATTERN_PONCTUATION  = r'[.,!?;:"\'«»()\[\]{}،؟؛…\-–—~`^*+=<>|\\/_]'
PATTERN_DETECT_PONCT = r'[.,!?;:"\'«»؟،؛]'
PATTERN_DETECT_HASHTAG = r'#'  # ✅ AJOUTÉ : détection des hashtags
...
def nettoyer_et_ecrire_partition(partition):
    ...
    re_urls        = re.compile(r'https?://\S*|www\.\S+', re.IGNORECASE)
    re_at          = re.compile(r'@')
    re_hashtag     = re.compile(r'#')  # ✅ AJOUTÉ
    re_ponctuation = re.compile(r'[.,!?;:"\'«»()\[\]{}،؟؛\u2026\u2013\u2014~`^*+=<>|\\/_]')
    re_espaces     = re.compile(r'\s+')
    ...
    def nettoyer_texte(texte):
        ...
        texte = re_urls.sub('', texte)
        texte = re_at.sub('', texte)
        texte = re_hashtag.sub('', texte)  # ✅ AJOUTÉ
        texte = re_ponctuation.sub(' ', texte)
        texte = re_espaces.sub(' ', texte).strip()
        return texte if texte else None
...
df_analyse = df_spark \
    .withColumn("a_url",   when(col("Commentaire_Client").rlike(PATTERN_DETECTION), 1).otherwise(0)) \
    .withColumn("a_at",    when(col("Commentaire_Client").rlike(PATTERN_AT), 1).otherwise(0)) \
    .withColumn("a_hashtag", when(col("Commentaire_Client").rlike(PATTERN_DETECT_HASHTAG), 1).otherwise(0)) \
    .withColumn("a_ponct", when(col("Commentaire_Client").rlike(PATTERN_DETECT_PONCT), 1).otherwise(0))
...
stats_avant = df_analyse.agg(
    ...
    spark_sum("a_url").alias("avec_urls"),
    spark_sum("a_at").alias("avec_at"),
    spark_sum("a_hashtag").alias("avec_hashtag"),  # ✅ AJOUTÉ
    spark_sum("a_ponct").alias("avec_ponct")
).collect()[0]
...
avec_urls_av  = int(stats_avant["avec_urls"]  or 0)
avec_at_av    = int(stats_avant["avec_at"]    or 0)
avec_hashtag_av = int(stats_avant["avec_hashtag"] or 0)  # ✅ AJOUTÉ
avec_ponct_av = int(stats_avant["avec_ponct"] or 0)
...
print(f"   │ Avec URLs             : {avec_urls_av:<15} │")
print(f"   │ Avec @                : {avec_at_av:<15} │")
print(f"   │ Avec #                : {avec_hashtag_av:<15} │")  # ✅ AJOUTÉ
print(f"   │ Avec ponctuations     : {avec_ponct_av:<15} │")
print(f"   │ % ponctuations        : {(avec_ponct_av/total*100):<14.2f}% │")
print(f"   └──────────────────────────────────────┘")
...
df_apres = df_nettoye \
    .withColumn("urls_ap",  when(col("Commentaire_Client").rlike(PATTERN_DETECTION), 1).otherwise(0)) \
    .withColumn("at_ap",    when(col("Commentaire_Client").rlike(PATTERN_AT), 1).otherwise(0)) \
    .withColumn("hashtag_ap", when(col("Commentaire_Client").rlike(PATTERN_DETECT_HASHTAG), 1).otherwise(0)) \
    .withColumn("ponct_ap", when(col("Commentaire_Client").rlike(PATTERN_DETECT_PONCT), 1).otherwise(0))

stats_apres = df_apres.agg(
    spark_sum("urls_ap").alias("avec_urls"),
    spark_sum("at_ap").alias("avec_at"),
    spark_sum("hashtag_ap").alias("avec_hashtag"),  # ✅ AJOUTÉ
    spark_sum("ponct_ap").alias("avec_ponct")
).collect()[0]

avec_urls_ap  = int(stats_apres["avec_urls"]  or 0)
avec_at_ap    = int(stats_apres["avec_at"]    or 0)
avec_hashtag_ap = int(stats_apres["avec_hashtag"] or 0)  # ✅ AJOUTÉ
avec_ponct_ap = int(stats_apres["avec_ponct"] or 0)
...
taux_urls  = ((avec_urls_av  - avec_urls_ap)  / avec_urls_av  * 100) if avec_urls_av  > 0 else 100.0
taux_at    = ((avec_at_av    - avec_at_ap)    / avec_at_av    * 100) if avec_at_av    > 0 else 100.0
taux_hashtag = ((avec_hashtag_av - avec_hashtag_ap) / avec_hashtag_av * 100) if avec_hashtag_av > 0 else 100.0  # ✅ AJOUTÉ
taux_ponct = ((avec_ponct_av - avec_ponct_ap) / avec_ponct_av * 100) if avec_ponct_av > 0 else 100.0
...
print(f"   │ @ supprimés           : {avec_at_av - avec_at_ap:<15} │")
print(f"   │ Taux @                : {taux_at:<14.2f}% │")
print(f"   │ # supprimés           : {avec_hashtag_av - avec_hashtag_ap:<15} │")  # ✅ AJOUTÉ
print(f"   │ Taux #                : {taux_hashtag:<14.2f}% │")  # ✅ AJOUTÉ
print(f"   │ Ponct. supprimées     : {avec_ponct_av - avec_ponct_ap:<15} │")
print(f"   │ Taux ponctuations     : {taux_ponct:<14.2f}% │")
print(f"   └──────────────────────────────────────┘")
...
urls_restantes  = coll_dest.count_documents({"Commentaire_Client": {"$regex": PATTERN_DETECTION, "$options": "i"}})
at_restants     = coll_dest.count_documents({"Commentaire_Client": {"$regex": "@"}})
hashtag_restants = coll_dest.count_documents({"Commentaire_Client": {"$regex": "#"}})  # ✅ AJOUTÉ
ponct_restantes = coll_dest.count_documents({"Commentaire_Client": {"$regex": "[.,!?;:]"}})
vides_restants  = coll_dest.count_documents({"$or": [
    {"Commentaire_Client": ""},
    {"Commentaire_Client": None},
...
temps_fin_verif = time.time()
succes = urls_restantes == 0 and at_restants == 0 and hashtag_restants == 0 and ponct_restantes == 0 and vides_restants == 0  # ✅ MODIFIÉ

print(f"   • Documents en destination  : {total_en_dest}")
print(f"   • URLs restantes            : {urls_restantes}")
print(f"   • @ restants                : {at_restants}")
print(f"   • # restants                : {hashtag_restants}")  # ✅ AJOUTÉ
print(f"   • Ponctuations restantes    : {ponct_restantes}")
print(f"   • Lignes vides restantes    : {vides_restants}")
print(f"   {'✅ SUCCÈS TOTAL !' if succes else '⚠️  Vérifier manuellement'}")
...
📊 RÉSULTATS:
   ...
   • URLs supprimées    : {avec_urls_av - avec_urls_ap} ({taux_urls:.1f}%)
   • @ supprimés        : {avec_at_av - avec_at_ap} ({taux_at:.1f}%)
   • # supprimés        : {avec_hashtag_av - avec_hashtag_ap} ({taux_hashtag:.1f}%)  # ✅ AJOUTÉ
   • Ponct. supprimées  : {avec_ponct_av - avec_ponct_ap} ({taux_ponct:.1f}%)
   • Lignes vides       : {total_vides} ignorées (non insérées)
   • Total inséré       : {total_inseres} documents
...
print(f"🗑️  URLs supprimées         : {avec_urls_av - avec_urls_ap} ({taux_urls:.1f}%)")
print(f"🗑️  @ supprimés             : {avec_at_av - avec_at_ap} ({taux_at:.1f}%)")
print(f"🗑️  # supprimés             : {avec_hashtag_av - avec_hashtag_ap} ({taux_hashtag:.1f}%)")  # ✅ AJOUTÉ
print(f"🗑️  Ponctuations supprimées : {avec_ponct_av - avec_ponct_ap} ({taux_ponct:.1f}%)")
print(f"🗑️  Lignes vides ignorées   : {total_vides}")
print(f"⏱️  Temps total             : {temps_total:.2f}s")
print(f"🚀 Vitesse                 : {total/temps_total:.0f} docs/s")
