...
# pages/2_Analyse.py - Version sans émoji, icônes uniquement
import streamlit as st
from pymongo import MongoClient
import pandas as pd
import plotly.express as px
...
st.set_page_config(
    page_title="Analyse - Algérie Télécom",
    page_icon="📊",  # emoji résiduel dans l'onglet navigateur (non modifiable facilement)
    layout="wide",
...
with st.sidebar:
    render_sidebar()
    st.markdown("---")
    st.markdown('<div class="menu-section-title"><i class="fas fa-filter"></i> FILTRES D\'ANALYSE</div>', unsafe_allow_html=True)
    ...
    st.markdown('<span class="filter-label"><i class="fas fa-smile"></i> Sentiment</span>', unsafe_allow_html=True)
    sentiment_filter = st.multiselect(
        "Choisir", ["positif", "negatif", "neutre"],
        default=["positif", "negatif"], label_visibility="collapsed", key="sentiment_analysis"
    )
    
    st.markdown('<span class="filter-label"><i class="fas fa-database"></i> Source</span>', unsafe_allow_html=True)
    sources_filter = st.multiselect(
        "Choisir", ["Facebook", "Twitter", "Forum", "Test_manuel"],
        default=["Facebook", "Twitter", "Forum", "Test_manuel"],
        label_visibility="collapsed", key="source_filter"
    ...
    if st.button("Appliquer les filtres", icon=":material/sync:", use_container_width=True, type="primary"):
        st.rerun()

# ------------------------------------------------------------
# FONCTIONS KPI & HEADER
# ------------------------------------------------------------
def kpi_card(icon, label, value, delta=None, delta_up=True):
    delta_html = ""
    if delta:
        sign = "▲" if delta_up else "▼"
        delta_class = "delta-up" if delta_up else "delta-down"
        delta_html = f'<div class="kpi-delta {delta_class}">{sign} {delta}</div>'
    return f"""
    <div class="kpi-card">
...
def page_header(icon, title, subtitle=""):
    return f"""
    <div style="background: linear-gradient(135deg, #0f3b5c 0%, #1e6b5e 100%); 
                border-radius: 24px; padding: 1.5rem; margin-bottom: 1.5rem; color: white;">
        <h1 style="margin:0"><i class="{icon}"></i> {title}</h1>
        <p style="margin-top:0.5rem; opacity:0.9">{subtitle}</p>
    </div>
    """
...
def load_data():
    ...
    if sources_filter:
        query["sources"] = {"$in": sources_filter}
    # Période (à adapter à votre schéma)
    if period != "Tout":
        days_map = {"7 derniers jours": 7, "30 derniers jours": 30, "3 derniers mois": 90}
        date_limit = datetime.now() - timedelta(days=days_map.get(period, 30))
        # query["dates"] = {"$gte": date_limit.strftime("%Y-%m-%d")}
    data = list(collection.find(query).limit(5000))
    df = pd.DataFrame(data) if data else pd.DataFrame()
    if not df.empty and 'dates' in df.columns:
        df['date_parsed'] = pd.to_datetime(df['dates'], errors='coerce')
...
if df.empty:
    st.warning("Aucune donnée disponible", icon=":material/warning:")
    st.stop()

# ------------------------------------------------------------
# PAGE PRINCIPALE
# ------------------------------------------------------------
st.markdown(page_header("fas fa-chart-line", "Analyse approfondie", 
                       "Évolutions, distributions et corrélations"), 
           unsafe_allow_html=True)
...
col1, col2, col3, col4 = st.columns(4)
with col1:
    st.markdown(kpi_card("fas fa-file-alt", "Total", f"{total:,}"), unsafe_allow_html=True)
with col2:
    taux_p = pos/total*100 if total else 0
    st.markdown(kpi_card("fas fa-smile", "Taux positif", f"{taux_p:.1f}%", 
                        delta=f"{pos:,}", delta_up=True), unsafe_allow_html=True)
with col3:
    taux_n = neg/total*100 if total else 0
    st.markdown(kpi_card("fas fa-frown", "Taux négatif", f"{taux_n:.1f}%", 
                        delta=f"{neg:,}", delta_up=False), unsafe_allow_html=True)
with col4:
    sources_count = df['sources'].nunique() if 'sources' in df.columns else 0
    st.markdown(kpi_card("fas fa-chart-line", "Sources", str(sources_count)), unsafe_allow_html=True)

st.markdown("<br>", unsafe_allow_html=True)

# ---- Évolution temporelle ----
st.markdown('<div class="card">', unsafe_allow_html=True)
st.markdown('<div class="card-title"><i class="fas fa-chart-line"></i> Évolution dans le temps</div>', unsafe_allow_html=True)
...
if not df.empty and 'date_parsed' in df.columns:
    ...
    if not df_clean.empty:
        df_clean['date_only'] = df_clean['date_parsed'].dt.date
        daily = df_clean.groupby(['date_only', 'label_final']).size().reset_index(name='count')
        color_map = {"positif": "#22c55e", "negatif": "#ef4444", "neutre": "#94a3b8"}
        fig = px.line(daily, x='date_only', y='count', color='label_final',
                      color_discrete_map=color_map, markers=True)
        fig.update_traces(line=dict(width=2.5), marker=dict(size=5))
        fig.update_layout(plot_bgcolor="white", height=320,
                          xaxis_title="Date", yaxis_title="Nombre de commentaires",
                          legend_title="Sentiment", margin=dict(l=0, r=0, t=0, b=0))
        st.plotly_chart(fig, use_container_width=True, config={"displayModeBar": True})
...
with c1:
    st.markdown('<div class="card">', unsafe_allow_html=True)
    st.markdown('<div class="card-title"><i class="fas fa-bullseye"></i> Distribution des sentiments</div>', unsafe_allow_html=True)
    if 'label_final' in df.columns:
        counts = df['label_final'].value_counts()
        fig_pie = px.pie(values=counts.values, names=counts.index,
                         color=counts.index, color_discrete_map=color_map, hole=0.5)
        fig_pie.update_traces(textinfo="percent+label", textfont_color="white")
        fig_pie.update_layout(height=280, margin=dict(l=0, r=0, t=0, b=0))
        st.plotly_chart(fig_pie, use_container_width=True, config={"displayModeBar": False})
    st.markdown("</div>", unsafe_allow_html=True)
...
with c2:
    if 'confidence' in df.columns and not df['confidence'].isna().all():
        st.markdown('<div class="card">', unsafe_allow_html=True)
        st.markdown('<div class="card-title"><i class="fas fa-chart-bar"></i> Scores de confiance</div>', unsafe_allow_html=True)
        fig_hist = px.histogram(df, x='confidence', color='label_final',
                                color_discrete_map=color_map, nbins=30, barmode='overlay', opacity=0.7)
        fig_hist.update_layout(height=280, xaxis_title="Score de confiance", yaxis_title="Fréquence",
...
if 'reason' in df.columns and not df['reason'].isna().all():
    st.markdown('<div class="card">', unsafe_allow_html=True)
    st.markdown('<div class="card-title"><i class="fas fa-search"></i> Top des raisons</div>', unsafe_allow_html=True)
    r1, r2 = st.columns(2)
    with r1:
        st.markdown('<div style="font-weight:600; color:#ef4444; margin-bottom:0.5rem;"><i class="fas fa-angry"></i> Négatives</div>', unsafe_allow_html=True)
        neg_reasons = df[df['label_final'] == 'negatif']['reason'].value_counts().head(5)
        for reason, count in neg_reasons.items():
            st.markdown(f"""
            <div style="display:flex; justify-content:space-between; padding:0.3rem 0; border-bottom:1px solid #f1f5f9;">
            ...
            """, unsafe_allow_html=True)
    with r2:
        st.markdown('<div style="font-weight:600; color:#22c55e; margin-bottom:0.5rem;"><i class="fas fa-smile"></i> Positives</div>', unsafe_allow_html=True)
        pos_reasons = df[df['label_final'] == 'positif']['reason'].value_counts().head(5)
        for reason, count in pos_reasons.items():
            st.markdown(f"""
            <div style="display:flex; justify-content:space-between; padding:0.3rem 0; border-bottom:1px solid #f1f5f9;">
...
# ---- Tableau détaillé ----
st.markdown('<div class="card">', unsafe_allow_html=True)
st.markdown('<div class="card-title"><i class="fas fa-table"></i> Données détaillées</div>', unsafe_allow_html=True)
display_cols = ['Commentaire_Client', 'label_final', 'sources', 'dates', 'confidence']
existing_cols = [c for c in display_cols if c in df.columns]
st.dataframe(df[existing_cols].head(50), use_container_width=True, hide_index=True,
             column_config={"label_final": st.column_config.SelectboxColumn("Sentiment", options=["positif", "negatif", "neutre"])})
csv = df[existing_cols].to_csv(index=False).encode('utf-8')
st.download_button(label="Exporter en CSV", data=csv,
                   file_name=f"analyse_telecom_{datetime.now().strftime('%Y%m%d')}.csv",
                   mime="text/csv", use_container_width=True, icon=":material/download:")
st.markdown("</div>", unsafe_allow_html=True)

# ---- Tests statistiques ----
with st.expander("Tests statistiques avancés", icon=":material/calculate:"):
    from scipy import stats
    if 'sources' in df.columns and 'label_final' in df.columns:
        st.markdown("**Test du χ² : Source ↔ Sentiment**")
        contingency = pd.crosstab(df['sources'], df['label_final'])
        ...
        with col_stat2:
            st.metric("p-value", f"{p_value:.4f}")
        if p_value < 0.05:
            st.success("Relation significative détectée (p < 0.05)", icon=":material/check_circle:")
        else:
            st.info("Pas de relation significative (p ≥ 0.05)", icon=":material/info:")

st.markdown("---")
st.caption('© 2026 Algérie Télécom - Analyse des sentiments en temps réel')