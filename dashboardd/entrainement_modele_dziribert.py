...
# # Chemins absolus des scripts (corrigés)
# BASE_DIR = "/home/mouna/projet_telecom/scripts/nettoyage"
# SCRIPTS = {
#     "etape_1": os.path.join(BASE_DIR, "Nettoyage_des_textes/code/suppression_urls.py"),
#     "etape_2": os.path.join(BASE_DIR, "Nettoyage_des_textes/code/combini_TF_JK_complet.py"),  # ← CORRIGÉ
#     "etape_3": os.path.join(BASE_DIR, "Nettoyage_des_textes/code/extraire_emojis_multinode.py"),
#     "etape_4": os.path.join(BASE_DIR, "Normalisation/code/normalisation_multinode.py"),
# }

# # Collections correspondantes (corrigées)
# COLLECTIONS = {
#     "source": "commentaires_bruts",
#     "etape_1": "commentaires_sans_urls_arobase",
#     "etape_2": "commentaires_sans_doublons_combinision",  # ← CORRIGÉ (nom exact)
#     "etape_3": "commentaires_sans_emojis",
#     "etape_4": "commentaires_normalises",
# }
...
# # ============================================================
# # FONCTIONS MONGODB (avec gestion des IDs string)
# # ============================================================
...
#         return count
#     except Exception as e:
#         print(f"⚠️ Erreur comptage {collection_name}: {e}")
#         return -1

# def inserer_commentaire_test():
#     """Insère un commentaire de test avec ID string (pas ObjectId)"""
#     import random
...
#     # ⚠️ IMPORTANT: Générer un ID string manuellement
#     # Soit en utilisant ObjectId converti en string
#     # Soit en générant un UUID
#     import uuid
#     string_id = str(uuid.uuid4()).replace("-", "")[:24]  # Génère un ID string de 24 caractères
#     # Ou plus simple: string_id = str(ObjectId())  # Convertit ObjectId en string
    
#     doc = {
#         "_id": string_id,  # ← STOCKÉ COMME STRING !
#         "Commentaire_Client": texte,
#         "source": "Test_manuel",
#         "statut": "brut",
#         "traite": False,
...
#     client.close()
#     print(f"✅ Commentaire test inséré: {texte[:50]}...")
#     print(f"   ID: {string_id} (stocké comme STRING)")
#     return string_id


# def marquer_comme_traites(collection_name, ids):
#     """Marque les commentaires comme traités - IDs peuvent être strings ou ObjectId"""
#     if not ids:
#         return 0
#     try:
#         client = MongoClient(MONGO_URI, serverSelectionTimeoutMS=3000)
...
#         # Convertir les IDs en ObjectId si nécessaire (car MongoDB stocke en ObjectId)
#         from bson.objectid import ObjectId
#         object_ids = []
#         for id_val in ids:
#             try:
#                 # Si c'est une string de 24 caractères hex, convertir en ObjectId
...
# def get_nouveaux_ids(collection_name):
#     """Récupère les IDs des documents avec traite=false (comme strings)"""
#     try:
#         client = MongoClient(MONGO_URI, serverSelectionTimeoutMS=3000)
#         db = client[DB_NAME]
#         collection = db[collection_name]
...
#         docs = list(collection.find(query, {"_id": 1}))
#         client.close()
#         # Retourner les IDs comme strings pour éviter les problèmes
#         return [str(doc["_id"]) for doc in docs]
#     except Exception as e:
#         print(f"⚠️ Erreur récupération IDs: {e}")
#         return []
...
#     stats = {}
#     for nom, coll in COLLECTIONS.items():
#         total = compter_docs(coll)
#         non_traites = compter_docs(coll, {"traite": False}) if coll != "commentaires_bruts" else "N/A"
#         stats[nom] = {"total": total, "non_traites": non_traites}
#         print(f"   {coll}: {total} docs (non traités: {non_traites})")
...
#     print("📝 Insertion d'un commentaire de test")
#     print("="*60)
#     inserted_id = inserer_commentaire_test()
...
#         raise Exception(f"Script étape 1 introuvable")
...
#     if result.stdout:
#         print(result.stdout[-500:])  # Afficher la fin de la sortie
#     if result.stderr:
...
#     # Récupérer les IDs traités et les marquer
#     ids = get_nouveaux_ids(COLLECTIONS["source"])
#     marques = marquer_comme_traites(COLLECTIONS["source"], ids)
#     print(f"   ✅ {marques} documents marqués traités")
...
# @task(name="Étape 2 — Suppression doublons (Combo B)", retries=1)
# def task_etape_2():
#     print("\n" + "="*60)
#     print("🔹 ÉTAPE 2 — Suppression des doublons (Combo B: TF-IDF + Jaccard Mots)")
#     print("="*60)
...
#     if not os.path.exists(script):
#         print(f"❌ Script non trouvé: {script}")
#         raise Exception(f"Script étape 2 introuvable")
...
# def task_etape_3():
#     print("\n" + "="*60)
#     print("🔹 ÉTAPE 3 — Extraction des emojis")
#     print("="*60)
...
#     script = SCRIPTS["etape_3"]
#     if not os.path.exists(script):
#         print(f"⚠️ Script étape 3 non trouvé: {script}")
#         print("   Passage à l'étape suivante...")
#         return {"apres": compter_docs(COLLECTIONS["etape_2"])}
...
#     if result.returncode != 0:
#         print("⚠️ Erreur étape 3, mais on continue...")
#         send_kafka_message("etape_3_erreur", {"error": result.stderr[-200:]})
#         return {"apres": compter_docs(COLLECTIONS["etape_2"])}
...
# def task_etape_4():
#     print("\n" + "="*60)
#     print("🔹 ÉTAPE 4 — Normalisation du texte")
#     print("="*60)
...
#     script = SCRIPTS["etape_4"]
#     if not os.path.exists(script):
#         print(f"⚠️ Script étape 4 non trouvé: {script}")
#         print("   Pipeline terminé sans normalisation.")
#         return {"apres": compter_docs(COLLECTIONS["etape_3"])}
...
#     if result.returncode != 0:
#         print("⚠️ Erreur étape 4, mais pipeline terminé")
#         send_kafka_message("etape_4_erreur", {"error": result.stderr[-200:]})
#         return {"apres": compter_docs(COLLECTIONS["etape_3"])}
...
#fih etape ta3 les flag :

#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# pipeline_complet.py
# Pipeline complet avec Prefect Dashboard + Insertion commentaire
# Inclut l'extraction des flags et la création de dataset_unifie
...
SCRIPTS = {
    "etape_1": os.path.join(BASE_DIR, "Nettoyage_des_textes/code/suppression_urls.py"),
    "etape_2": os.path.join(BASE_DIR, "Nettoyage_des_textes/code/combini_TF_JK_complet.py"),
    "etape_3": os.path.join(BASE_DIR, "Nettoyage_des_textes/code/extraire_emojis_multinode.py"),
    "etape_4": os.path.join(BASE_DIR, "Normalisation/code/normalisation_multinode.py"),
    "etape_5": os.path.join(BASE_DIR, "Flags/flags_complet.py"),  # ← NOUVEAU : Extraction des flags
}
...
COLLECTIONS = {
    "source": "commentaires_bruts",
    "etape_1": "commentaires_sans_urls_arobase",
    "etape_2": "commentaires_sans_doublons_combinision",
    "etape_3": "commentaires_sans_emojis",
    "etape_4": "commentaires_normalises",
    "etape_5": "dataset_unifie",  # ← NOUVEAU : Collection finale
}
...
def inserer_commentaire_test():
    """Insère un commentaire de test avec ID string"""
    import random
    import uuid
    
    commentaires_test = [
        "💚 دائما أقرب",
        "الخدمة ممتازة شكرا لكم",
        "Très bon service !",
        "Problème de connexion depuis 3 jours"
    ]
    texte = random.choice(commentaires_test)
    ...
    string_id = str(uuid.uuid4()).replace("-", "")[:24]
...
def get_stats():
    ...
    for nom, coll in COLLECTIONS.items():
        total = compter_docs(coll)
        non_traites = compter_docs(coll, {"traite": False}) if coll not in ["commentaires_bruts", "dataset_unifie"] else "N/A"
        stats[nom] = {"total": total, "non_traites": non_traites}
        print(f"   {coll}: {total} docs (non traités: {non_traites})")
...
@task(name="Étape 5 — Extraction des flags", retries=1)
def task_etape_5():
    """
    ÉTAPE 5: Extraction des flags (social, suggestion, plainte, negation, mixte, encouragement)
    Lit les commentaires avec traite=false dans commentaires_normalises
    Ajoute les flags et stocke dans dataset_unifie
    Marque traite=true dans commentaires_normalises
    """
    print("\n" + "="*60)
    print("🔹 ÉTAPE 5 — Extraction des flags")
    print("   (social, suggestion, plainte, negation, mixte, encouragement)")
    print("="*60)
    ...
    if not os.path.exists(script):
        print(f"⚠️ Script étape 5 non trouvé: {script}")
        print("   Pipeline terminé sans extraction des flags.")
        return {"apres": compter_docs(COLLECTIONS["etape_4"])}
    ...
    if result.returncode != 0:
        print("⚠️ Erreur étape 5, mais pipeline terminé")
    ...
    send_kafka_message("etape_5_fin", {"apres": apres, "duree": duree})
    print(f"✅ Terminé en {duree:.1f}s")
    print(f"   📊 Dataset unifié: {apres} commentaires avec flags")
    return {"apres": apres}
...
def pipeline_complet():
    ...
    # 2. Exécuter les 5 étapes
    task_etape_1()   # Suppression URLs
    task_etape_2()   # Suppression doublons
    task_etape_3()   # Extraction emojis
    task_etape_4()   # Normalisation
    task_etape_5()   # Extraction des flags + création dataset_unifie
    ...
    print("   ✅ Étape 3: Extraction des emojis")
    print("   ✅ Étape 4: Normalisation du texte")
    print("   ✅ Étape 5: Extraction des flags → dataset_unifie")
    print("="*60)
